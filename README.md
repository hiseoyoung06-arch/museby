...
- **뮤즈바이 관리자** (`/admin/dashboard`): 로그인 없이 바로 접근 가능 (임시 상태, 아래
  "알려진 제한 사항" 참고)
...
- 브랜드 입장은 계정 시스템 없이 HMAC 서명된 쿠키 세션을 사용합니다
  (Supabase Auth 미사용).
...
- `next@14.2.35`를 사용합니다. `npm audit`에 남아있는 Next.js 관련 항목들은
  ...
  보류했습니다).
- **`/admin` 이하 전체가 현재 로그인 없이 접근 가능합니다.** 인플루언서의 이름·연락처·
  주소가 담긴 화면이라 URL만 알면 누구나 볼 수 있는 상태예요. 임시로 요청받아 비활성화한
  것이며, 운영 전에는 반드시 비밀번호나 계정 기반 접근 제어를 다시 추가해야 합니다.
