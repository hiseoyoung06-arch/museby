...
- **뮤즈바이 관리자** (`/admin/login`): 이메일/비밀번호로 로그인
...
- Supabase (Postgres + Auth) — 서버 액션에서 서비스 롤 키로만 데이터에 접근하고,
...
3. **뮤즈바이 관리자 계정**은 Supabase Dashboard → Authentication → Users에서
   직접 생성합니다. (별도 회원가입 화면은 제공하지 않습니다.)
...
- `next@14.2.35`를 사용합니다. `npm audit`에 남아있는 Next.js 관련 항목들은
  ...
  보류했습니다).
- 뮤즈바이 관리자 계정은 이메일 화이트리스트 없이 "로그인에 성공한 모든
  Supabase Auth 사용자 = 관리자"로 취급합니다. 공개 회원가입 화면이 없으므로
  Supabase Dashboard에서 직접 만든 계정만 로그인할 수 있습니다.
